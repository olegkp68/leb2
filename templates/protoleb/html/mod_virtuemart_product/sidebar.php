        ...
        <ul class="vmproduct<?php echo $params->get('moduleclass_sfx'); ?> productdetails">
            <?php foreach ($products as $product) : ?>
                ...
                <li class="<?php echo $pwidth ?> <?php echo $float ?>">
                    <?php
                    $url = JRoute::_('index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id=' . $product->virtuemart_product_id . '&virtuemart_category_id=' . $product->virtuemart_category_id); ?>
                    <h2><a href="<?php echo $url ?>" class="mod_vm_link" rel="tooltip" title="<?php echo $product->product_name ?> - <?php echo $product->product_s_desc ;?>"><?php echo $product->product_name ?></a><br>Артикул: <span style="color:#C4013D"><?php echo $product->product_sku ?></span></h2>
                    <?php echo '<div class="clear"></div>';
