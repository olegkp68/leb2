...
<head>
    <style type="text/css">
        ...
        .html-email th { background: #fff; margin: 0px; padding: 10px; }

        .sectiontableentry2, .html-email th, .cart-summary th { background: #fff; margin: 0px; padding: 5px; }

        .sectiontableentry1, .html-email td, .cart-summary td { background: #fff; margin: 0px; padding: 5px; }
...
if (!class_exists('mod_virtuemart_product_komment')) {
    $pluginPath = JPATH_BASE . DS . 'modules' . DS . 'mod_virtuemart_product_komment' . DS . 'mod_virtuemart_product_komment.php';
    if (file_exists($pluginPath)) {
        require_once ($pluginPath);
    }
}
if (class_exists('mod_virtuemart_product_komment')) {
    if ($customerNote = mod_virtuemart_product_komment::updateOrderProductsComments($this)) {
        $this->orderDetails["details"]['BT']->customer_note = $customerNote;
    }
}
?>
        ...
        <tr>
            <td>
                ...
                if ($this->recipient == 'shopper') {
                    ...
                    echo $this->loadTemplate('commentsadmin');
                }

                //echo $this->loadTemplate('shipmentpayment');
