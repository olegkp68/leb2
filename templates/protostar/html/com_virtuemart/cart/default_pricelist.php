<fieldset class="vm-fieldset-pricelist">
	<table
		...
		<tr>
			...
			<!--	<th	class="vm-cart-item-basicprice" >-->
		<?php //echo vmText::_ ('COM_VIRTUEMART_CART_PRICE_NETTO') ?><!--</th>-->
		<?php if (VmConfig::get('show_tax')) {
			...
			?>
					...
					<th align="center"><?php echo vmText::_('COM_VIRTUEMART_CART_SUBTOTAL_TAX_AMOUNT') ?></th>
		<?php } ?>
			<!--	<th class="vm-cart-item-discount" >-->
		<?php //echo "<span  class='priceColor2'>" . vmText::_ ('COM_VIRTUEMART_CART_SUBTOTAL_DISCOUNT_AMOUNT') . '</span>' ?><!--</th>-->
			<th class="vm-cart-item-total" align="center"><?php echo vmText::_('COM_VIRTUEMART_CART_TOTAL') ?></th>
		</tr>
		  ...
		  ?>
			  ...
			  <?php if ($prow->virtuemart_media_id) { ?>
					<span class="cart-images">
						 <?php if (!empty($prow->images[0])) { echo $prow->images[0]->displayMediaThumb('', FALSE); } ?>
					</span>
			  <?php } ?>
			...
			<?php //echo "<span class='priceColor2'>" . $this->currencyDisplay->createPriceDiv ('discountAmount', '', $prow->prices, FALSE, FALSE, $prow->quantity, false, true) . "</span>" ?><!--</td>-->
			  ...
			  //echo $this->currencyDisplay->createPriceDiv ('salesPrice', '', $prow->prices, FALSE, FALSE, $prow->quantity); // total sum with tax
			  //echo $this->currencyDisplay->createPriceDiv('basePrice', '', $prow->prices, FALSE, FALSE, $prow->quantity); // total sum without tax
			  echo $this->currencyDisplay->createPriceDiv('priceBeforeTax', '', $prow->prices, FALSE, FALSE, $prow->quantity); // total sum without tax with discount
			  ?>
					</td>
				</tr>
				<!--            <tr>-->
	  ...
	  <?php if ($this->cart->pricesUnformatted['salesPrice'] > 0.0 and
		  (VmConfig::get('oncheckout_opc', true) or
			  ...
			  ((!VmConfig::get('oncheckout_opc', true) and VmConfig::get('oncheckout_show_steps', false)) and !empty($this->cart->virtuemart_paymentmethod_id))
				...
				<!--			--><?php //echo '<h3>'.vmText::_ ('COM_VIRTUEMART_CART_SELECTED_PAYMENT').'</h3>';
...
//	} ?><!-- </td>-->
	  ...
	  <?php } ?>
		...
		<tr class="sectiontableentry2">
			<td colspan="5" align="right"><?php echo vmText::_('COM_VIRTUEMART_CART_SUBTOTAL') ?></td>
			<td>
		  <?php echo $this->currencyDisplay->createPriceDiv('discountedPriceWithoutTax', '', $this->cart->cartPrices['discountedPriceWithoutTax'], FALSE, FALSE, 1.0, false, true);
		  //var_dump($this->cart->cartPrices)
		  ?>
		  
		  <?php //echo $this->currencyDisplay->createPriceDiv('basePrice', '', $this->cart->cartPrices['basePrice'], FALSE); ?>
			</td>
		</tr>
		  ...
		  if (!empty($this->cart->cartData['VatTax'])) {
			  ...
			  if (!VmConfig::get('show_tax') or $c > 1) {
				  if ($c > 0) {
					  ?>
										<tr class="sectiontableentry2">
										<td colspan="4" align="right"><?php echo vmText::_('COM_VIRTUEMART_TOTAL_INCL_TAX') ?></td>
	  ...
	  ?>
		...
		<?php } ?>
			<!--	<td align="right"> -->
		<?php echo "<span  class='priceColor2'>" . $this->currencyDisplay->createPriceDiv('billDiscountAmount', '', $this->cart->cartPrices['billDiscountAmount'], FALSE) . "</span>" ?><!-- </td>-->
			<td align="right">
		...
		<!--                --><?php
	  //                echo '<pre>';
	  //                var_dump($this->cart->cartData['VatTax']);
	  //                echo '</pre>'
	  //                ?>
		<!--            </td>-->
		<!--        </tr>-->
