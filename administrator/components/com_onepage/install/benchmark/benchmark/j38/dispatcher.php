...
class JEventDispatcher extends JObject
{
	...
	public function trigger($event, $args = array())
	{
		...
		global $bench_arr; 
		if (empty($bench_arr['all_triggers'])) $bench_arr['all_triggers'] = array(); 
		
		if (empty($bench_arr['all_triggers'][$event])) {
		$b = debug_backtrace(); 
		$r = array(); 
		foreach ($b as $x) {
		...
		$bench_arr['all_triggers'][$event] = $r; 
		}
		...
		foreach ($this->_methods[$event] as $key)
		{
  		  ...
  		  if (empty($bench_arr)) $bench_arr = array(); 
		  $bench['start'] = microtime(true); 
		  
		 
		  if ((is_array($this->_observers[$key]) && (is_string($this->_observers[$key]['handler'] )))) {
		   $nameKey = $bench['name'] = $this->_observers[$key]['handler']; 
		   $className = 'joomla core'; 
		  }
		  else
		  {
			 if (is_object($this->_observers[$key])) {
			  $className = get_class($this->_observers[$key]); 
			 }
			 else
			 {
				 $className = 'joomla core'; 
			 }
		    $nameKey =  $bench['name'] = $className.'->'.$event; 
		  }
		    ...
		    $bench['duration'] = (float)$bench['end'] - (float)$bench['start']; 
			$bench['all'] = array(); 
		    if (isset($GLOBALS['bench_arr']['plugins'][$className]))
			{
				
				
				$benchOld = $GLOBALS['bench_arr']['plugins'][$className]['all']; 
			...
			$GLOBALS['bench_arr']['plugins'][$className] = $bench; 
