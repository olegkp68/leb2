...
abstract class PluginHelper
{
	...
	protected static function import($plugin, $autocreate = true, \JEventDispatcher $dispatcher = null)
	{
		...
		if (isset($GLOBALS['bench_arr']['pluginload'][$name]))
			{
				
				
				$benchOld = $GLOBALS['bench_arr']['pluginload'][$name]['all']; 
		...
		$GLOBALS['bench_arr']['pluginload'][$name] = $bench; 
		
		if ($plugin->name === 'benchmark') {
