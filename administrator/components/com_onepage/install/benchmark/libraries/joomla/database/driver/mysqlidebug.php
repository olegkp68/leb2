 ...
 * @since  3.0.0
 */
class JDatabaseDriverMysqlidebug extends JDatabaseDriver
{
	/**
	...
	public function connect()
	{
		...
		
		{
			...
			{
				mysqli_query($this->connection, 'SET profiling_history_size = 10000, profiling = 1;');
			}
		}
	}
	...
	public function execute()
	{
		...
		
		{
			...
			$obj->l = $length; 
			$mem  = memory_get_usage() - $memoryBefore;
			$obj->m = $mem; 
			$bx = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS);
			//reverse:for ($i=count($b) -1; $i>0; $i-- ) { //$x = $b[$i]; 
			
			$obj->b = array(); 
			foreach($bx as $x) {
				
				if ($this->backtraceOK($x)) {
					$b = $x['file'].':'.$x['line']; 
				}
			...
			$obj->b[] = $b; 
			if (empty($this->log[$hash])) {
			  $obj->c = 1; 
			  $this->log[$hash] = $obj; 
			}
			else {
				
				if (empty($this->log[$hash]->other)) $this->log[$hash]->other = array(); 
				$current = $this->log[$hash]; 
				if (count($this->log) < 100) {
				 $current->other[] = $obj; 
				}
				
				$current->m = (int)$current->m;
				$current->l = (float)$current->l;
				$current->l += $length; 
				$current->m += $obj->m; 
				$current->c++; 
				
				$this->log[$hash] = $current; 
			}

			
		}
		
		$qtest = preg_replace('/[0-9,]+/', '', $query);
		if ($qtest !== $query) {
		$h2 = md5($qtest); 
		$obj = new stdClass(); 
		$obj->q = 'DENUMBERED:'.$qtest; 
		$obj->c = 1; 
		$obj->l = $length; 
		$obj->m = $mem; 
		if (isset($this->log['m_'.$h2])) {
			$c = $this->log['m_'.$h2]; 
			$c->l += $obj->l; 
			$c->m += $obj->m; 
			$c->c++; 
	...
	private function backtraceOK($x) {
		if (!empty($x['file']))
				if (strpos($x['file'], 'table')) {
					return false; 
				}
				else
				if (strpos($x['file'], 'vmmodel')) {
					return false;
				}
				else
				if (strpos($x['file'], DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR)!==false) {
					$b = $x['file'].':'.$x['line']; 
					return true; 
				}
				else
				if (strpos($x['file'], DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR)!==false) {
					$za = explode(DIRECTORY_SEPARATOR,$x['file']);
					foreach ($za as $ind=>$p) {
						if (strpos($p, 'com_') === 0) {
							$component = substr($p, 4); 
							if (isset($za[$ind+1])) {
							if ($za[$ind+1] === $component.'.php') return false; 
							}
						}
					}
					$b = $x['file'].':'.$x['line']; 
				...
				if (strpos($x['file'], DIRECTORY_SEPARATOR.'plugins'.DIRECTORY_SEPARATOR)!==false) {
					$b = $x['file'].':'.$x['line']; 
					return true; 
				}
				else
				if (strpos($x['file'], DIRECTORY_SEPARATOR.'modules'.DIRECTORY_SEPARATOR)!==false) {
					$b = $x['file'].':'.$x['line']; 
					return true; 
				}
				return false; 
