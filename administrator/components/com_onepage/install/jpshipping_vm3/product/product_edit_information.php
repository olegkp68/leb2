...
$db = JFactory::getDBO(); 
$q = 'select `shipment_params` from #__virtuemart_shipmentmethods where `shipment_element` = \'jpshipping\' and published = 1'; 
$db->setQuery($q); 
...
if (!empty($rows)) 
foreach ($rows as $row) {
	$params = $row['shipment_params']; 
	if (!empty($params)) {
		$ex = explode('|', $params); 
		foreach ($ex as $line) {
			if (strpos($line, 'shipping_field_id') !== false) {
				$ex2 = explode('=', $line); 
				$id = json_decode($ex2[1]); 
				$id = (int)$id; 
				if (!empty($id)) {
					$shipping_ids[$id] = $id; 
				}
...
<fieldset>
    ...
    <table class="adminform" width="100%">
        ...
        <tr>
			...
			<td>
				<span class="hasTip" title="Shipping">
			...
			<td>
				...
				foreach ($this->product->customfields as $k=>$customfield) {
					...
					$custom_id = (int)$customfield->virtuemart_custom_id; 
					if (in_array($custom_id, $shipping_ids)) {
						
						$customfield->display = $customfieldsModel->displayProductCustomfieldBE ($customfield, $this->product, $i);
						$found = true; 
						$customfield->display = str_replace('</td><td>', '', $customfield->display); 
						echo $customfield->display.'<br />'; 
...
<?php 
foreach ($shipping_ids as $my_id) {
$i++; 	
?>
						<input type="hidden" value="S" name="field[<?php echo $i; ?>][field_type]">
			<input type="hidden" value="0" name="field[<?php echo $i; ?>][custom_element]">
			<input type="hidden" value="0" name="field[<?php echo $i; ?>][custom_jplugin_id]">
			<input type="hidden" value="<?php echo (int)$my_id; ?>" name="field[<?php echo $i; ?>][virtuemart_custom_id]">
			<input type="hidden" value="<?php echo (int)$this->product->virtuemart_product_id; ?>" name="field[<?php echo $i; ?>][virtuemart_product_id]">
			<input type="hidden" value="0" name="field[<?php echo $i; ?>][virtuemart_customfield_id]">
			<input class="ordering" type="hidden" value="0" name="field[<?php echo $i; ?>][ordering]">
					 	
					<input type="text" value="0.0" name="field[<?php echo $i; ?>][customfield_value]">
					<br />
					<?php
				}
				}
