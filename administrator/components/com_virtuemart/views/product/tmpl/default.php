...
if ($product_parent_id = vRequest::getInt('product_parent_id', false))   $col_product_name = 'COM_VIRTUEMART_PRODUCT_CHILDREN_LIST';
else $col_product_name = 'COM_VIRTUEMART_PRODUCT_NAME';
		...
		<span id="filterbox">
			<span>
				...
				<?php echo JHtml::_(
					...
					'class="inputbox" onchange="document.adminForm.submit(); return false;"',
					'value',
					'text',
			...
			<thead>
				<tr>
					...
					<?php if (!$product_parent_id) { ?>
						<th width="7%"><?php echo $this->sort('product_parent_id', 'COM_VIRTUEMART_PRODUCT_CHILDREN_OF'); ?></th>
					<?php } ?>
					<th width="60px"><?php echo vmText::_('COM_VIRTUEMART_PRODUCT_PARENT_LIST_CHILDREN'); ?></th>
					<th style="min-width:<?php echo $imgWidth ?>px;width:5%;"><?php echo vmText::_('COM_VIRTUEMART_PRODUCT_MEDIA'); ?></th>
					<th class="color"><?php echo $this->sort('`p`.product_sku', 'COM_VIRTUEMART_PRODUCT_SKU') ?></th>
					...
					<th width="10px"><?php echo $this->sort('product_special', 'COM_VIRTUEMART_PRODUCT_FORM_SPECIAL'); ?> </th>
					<th class="publ" width="40px"><?php echo $this->sort('published'); ?></th>
					<th width="40px"><?php echo 'Снято/<br>Опубл.'; ?> </th>
					<th class="color"><?php echo $this->sort('p.virtuemart_product_id', 'COM_VIRTUEMART_ID')  ?></th>
				</tr>
			</thead>
			<tbody>
				<?php
				...
				?>
						<tr class="row<?php echo $k; ?>">
							...
							<!-- Product SKU -->
							<td class="color"><?php echo $product->product_sku; ?></td>
							<!-- Product price -->
							<td align="right"><?php
							...
							*/ ?>
							...
							<td class="is_featured">
								<?php
								echo $is_featured;
								?>
							</td>
							<!-- published -->
							<td class="publ"><?php echo $published; ?></td>
							<td align="center"><?php echo $product->locked_on; ?></td>
							<!-- Vendor name -->
							<td class="color" align="right"><?php echo $product->virtuemart_product_id; // echo $product->vendor_name; 
																		?></td>
						</tr>
				<?php
						$k = 1 - $k;
