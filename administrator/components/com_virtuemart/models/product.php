	...
	static function getValidProductFilterArray()
	{
		...
		if (!isset($filterArray)) {
			...
			$filterArray = array(
				...
				'`p`.product_unit', '`p`.product_packaging', '`p`.virtuemart_product_id', 'pc.ordering', '`p`.locked_on'
			);
	...
	function sortSearchListQuery($onlyPublished = TRUE, $virtuemart_category_id = FALSE, $group = FALSE, $nbrReturnProducts = FALSE, $langFields = array())
	{
		...
		switch ($this->filter_order) {
			...
			case '`p`.created_on':
				...
				break;
			case 'published':
				$orderBy = ' ORDER BY p.`published` ' . $filterOrderDir . ', p.`locked_on` ' . $filterOrderDir;
				break;
			default:
