...
class UpdateHelper extends acymObject
{
    ...
    public function addUpdateSite($extension = null)
    {
        ...
        $object->location = ACYM_UPDATEMEURL.'updatexml&version=7.6.1';

        if (empty($extension)) {
            $type = 'component';
            $folder = '';
            $element = 'com_acym';
            $object->location .= '&component=acymailing&cms=joomla&level=enterprise';
    ...
    public function installTemplates($checkBeforeInstall = false)
    {
        ...
        foreach ($names as $name) {
            ...
            if ($checkBeforeInstall) {
                $oneMail = $mailClass->getOneByName($mailName, false, $mailClass::TYPE_TEMPLATE);
            ...
            $query = 'INSERT INTO `#__acym_mail` (`name`, `creation_date`, `thumbnail`, `drag_editor`, `type`, `body`, `subject`, `from_name`, `from_email`, `reply_to_name`, `reply_to_email`, `bcc`, `settings`, `stylesheet`, `attachments`, `creator_id`) VALUES
                     ('.$tmplName.', '.$creationDate.', '.$thumbnail.', 1, '.acym_escapeDB($mailClass::TYPE_TEMPLATE).', '.acym_escapeDB(
                    $template->body
