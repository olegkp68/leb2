								...
								<tr>
									...
									?>
										<th width="15%" class="nowrap login_as_user">
											<?php echo JHtml::_('searchtools.sort', 'Login as User', 'a.name', $listDirn, $listOrder); ?>
										</th>
									<?php 
									endif;
									// END: Web357 (Login as User - system joomla! plugin)
								...
								<tr class="row<?php echo $i % 2; ?>">
									...
									$auto_login_url = $live_site_url.'index.php?loginasclient=1&lacusr='.$this->escape(rawurlencode($item->username)).'&lacpas='.$this->escape($item->password);							
										if ($canChange && canLoginAsUser($item->id)):
											?>
											<td class="break-word login_as_user"><a href="<?php echo $auto_login_url; ?>" target="_blank" class="login_as_user_link"><span class="icon-user"></span> <?php echo sprintf($displayed_text, "<strong>".$this->escape($item->username)."</strong>"); ?></a></th>
											<?php 
										else:
											?>
