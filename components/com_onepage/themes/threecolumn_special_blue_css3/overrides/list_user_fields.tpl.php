   ...
   $default_shipping_country = $default_country_array[$current_lang->_lang];
		...
		$delimiter = 0;
	   	...
	   	foreach( $rowFields['fields'] as $field) {
			...
			$field['formcode'] = str_replace('type="', ' title="'.htmlentities($field['title']).'" alt="'.htmlentities($field['title']).'" placeholder="'.htmlentities($field['placeholder']).'" type="', $field['formcode']); 
	   			...
	   			default:
	   				...
	   				switch( $field['type'] ) {
						...
						case 'select':
							...
							//echo '<div class="before_select"></div><div class="middle_select" style=""><div class="after_select" style="" >&nbsp;</div>';
							$field['formcode'] = str_replace('style=', 'disabledstyle=', $field['formcode']); 
							echo $field['formcode']; 
							//echo '</div>';
							break;
					  case 'multiselect':
