...
$query->select('komment')
    ->from($db->quoteName('leb_vm_komment'));
$komment = $db->setQuery($query)->loadResult();
/*99999999999999999999*/
...
<a href="<?=JRoute::_( 'index.php?option=com_virtuemart&view=cart&task=click_removeall' )?>" id="click_removeall" class="click_removeall">Очистить корзину</a>
...
$query->select('komment')
    ->from($db->quoteName('leb_vm_komment'))
	->where($db->quoteName('prod_id') . ' = '.$product['product_id'])
...
?>
  <div class="op_basket_row">
    <div class="op_col1"><?php echo $this->op_show_image($product['product_full_image'], '', 70, 70, 'product'); ?>&nbsp;</div>
    <div class="op_col2_2"><?php echo $product['product_name'] . $product['product_attributes'].'<div class="komment">'.$komment.'</div>' ?>&nbsp;</div>
...
<div id="subtotal_box" <?php if(empty($op_coupon)) echo 'style="width:99%;"'; ?>>
