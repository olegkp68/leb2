...
//default code for VM to get the currency:
...
$currency_id = OPCLoader::getCurrency($cart);

$db = JFactory::getDBO();
            $q = 'select `currency_code_3` from #__virtuemart_currencies where virtuemart_currency_id = '.(int)$virtuemart_currency_id.' limit 0,1';
            $db->setQuery($q);
...
?><select name="<?php echo $field['name']; ?>" id="virtuemart_country_id"><option value="<?php

$q = 'select `virtuemart_country_id`, `country_name` from #__virtuemart_countries where country_2_code = \'CZ\' limit 1';

$db->setQuery($q);

$res= $db->loadAssoc();

$country_id = (int)$res['virtuemart_country_id'];

$country_name = $res['country_name'];

echo $country_id; ?>"><?php echo htmlentities($country_name); ?></option></select><?php

break;

default: echo $field['formcode'];
