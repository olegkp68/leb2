...
$has_guest_tab = false; 
if ((!empty($config)) && (isset($config->has_guest_tab)))
{
$has_guest_tab = $config->has_guest_tab; 
}	

$VM_REGISTRATION_TYPE = VM_REGISTRATION_TYPE; 
if (!empty($has_guest_tab)) 
{
$VM_REGISTRATION_TYPE = 'NORMAL_REGISTRATION'; 
$op_create_account_unchecked = false; 
		...
		$delimiter = 0;
	   	...
	   	foreach( $rowFields['fields'] as $field) {
			...
			if (!empty($field['required'])) $field['title'] .= ' *'; 
