...
if ((!(empty($no_login_in_template) && (VM_REGISTRATION_TYPE != 'NO_REGISTRATION'))) && (empty($registraton_html))) echo ' style="display: none;" '; 
	...
	<?php if (empty($no_login_in_template) && (VM_REGISTRATION_TYPE != 'NO_REGISTRATION')) { ?>
		<div id="logintab" class="tabcontent3" style="display: none;">
	      ...
	      <div class="wrap_login" style="float: none;">
	    			<div class="formLabel">
			   <?php if(!$isPlaceholder) echo '<label>'.OPCLang::_('COM_VIRTUEMART_USERNAME').'</label>'; ?>
			</div>
			<div class="formField">
			  <input type="text" id="username_login" name="username_login" class="inputbox" size="20" autocomplete="off" title="<?php echo OPCLang::_('COM_VIRTUEMART_USERNAME');?>" placeholder="<?php if($isPlaceholder) echo OPCLang::_('COM_VIRTUEMART_USERNAME');?>" />
			</div>
			<div class="formLabel">
			   <?php if(!$isPlaceholder) echo '<label>'.OPCLang::_('COM_VIRTUEMART_SHOPPER_FORM_PASSWORD_1').'</label>'; ?>
			</div>
			<div class="formField">
				...
				?>" class="inputbox" size="20" onkeypress="return submitenter(this,event)"  autocomplete="off" title="<?php echo OPCLang::_('COM_VIRTUEMART_SHOPPER_FORM_PASSWORD_1');?>" placeholder="<?php if($isPlaceholder) echo OPCLang::_('COM_VIRTUEMART_SHOPPER_FORM_PASSWORD_1');?>"/>
			</div>
	 
	<?php if (JPluginHelper::isEnabled('system', 'remember')) { ?>
	
	<div class="formLabel"><?php if(!$isPlaceholder) echo '<label></label>'; ?></div>
	<div class="formField">
	...
	<?php } ?>
	<div class="opc_login_btn_wrapper" style="" >
	<div class="opc_login_button" style="">
	<input type="button" name="LoginSubmit" class="buttonopc" value="<?php echo OPCLang::_('COM_VIRTUEMART_LOGIN'); ?>" onclick="javascript: return Onepage.op_login();"/>
	</div>
...
<div class="tabcontent3"><fieldset style="padding:10px;">
...
<?php } ?>
	<div id="agreed_div" class=" ">
		<input type="checkbox" id="agreed_field" name="agreed" value="1" class="inputbox" <?php if (!empty($agree_checked)) echo ' checked="checked" '; ?> class="terms-of-service"  required="required" autocomplete="off" />
					...
					<?php JHTMLOPC::_('behavior.modal', 'a.opcmodal'); 
					?><a target="_blank" rel="{handler: 'iframe', size: {x: 500, y: 400}}" class="opcmodal" href="<?php echo $tos_link; ?>" title="<?php  echo OPCLang::_('COM_VIRTUEMART_I_AGREE_TO_TOS'); ?>" onclick="javascript: return op_openlink(this); ">
...
<?php 
 ...
 ?>
</legend><div id="customer_note_div" class="formLabel" style="max-width:95% !important; margin:10px;">
<div id="customer_note_input" ><textarea placeholder="<?php echo $comment; ?>" name="customer_note" id="customer_note_field"></textarea></div>
</div>
...
*/
if (!empty($opc_rtl)) echo '<div style="display: none;">'; 
?>
