...
$isPlaceholder = true; 
		...
		$delimiter = 0;
		   ...
		   if (!empty($field['required']))
			...
			if ($isPlaceholder) { $field['formcode'] = str_replace('type="', ' title="'.$field['title'].'" alt="'.$field['title'].'" placeholder="'.$field['title'].'" type="', $field['formcode']); }
			 else { $field['formcode'] = str_replace('type="', ' title="'.$field['title'].'" alt="'.$field['title'].'" type="', $field['formcode']); }
		    ...
		    $readonly = $field['readonly'] ? ' readonly="readonly"' : '';
	   		...
	   		if (( $field['name'] == 'username' && VM_REGISTRATION_TYPE == 'OPTIONAL_REGISTRATION' ) || (( $field['name'] == 'opc_password' && VM_REGISTRATION_TYPE == 'OPTIONAL_REGISTRATION' && (!empty($op_usernameisemail)) ))) {
				echo '<div class="formLabel">
						<input type="checkbox" autocomplete="off" id="register_account" name="register_account" value="1" class="inputbox checkbox inline" onclick="return Onepage.showFields( this.checked, new Array('; 
...
';
			...
			if (($isPlaceholder) && (in_array($field['type'], $arr)))
			{
		  ...
		  echo '>'."\n";
			...
			if ($isPlaceholder) $field['formcode'] = str_replace('type="', ' alt="'.$field['title'].'" placeholder="'.$field['title'].'" type="', $field['formcode']); 
	      	 else $field['formcode'] = str_replace('type="', ' alt="'.$field['title'].'"  type="', $field['formcode']); 
			echo ' </div>'; 
			}
